	...
	document.getElementById("mmult").onclick = function() {
		...
		if (k <= 2) {
			...
			for (var j=0; j<3; j++) {
				...
				m2[j].style.backgroundColor="hsl(190, 70%, 85%)";			
				rowsum += m1[j+m].value*m2[j].value ;
				summaryM1[j].textContent=m1[j+m].value;
				summaryM2[j].textContent=m2[j].value;
				summaryM1M2[j].textContent = m1[j+m].value*m2[j].value;
